...
const fs = require('fs');
const config = require('./config.json'); // Требуется файл config.json с TELEGRAM_BOT_TOKEN и ADMIN_CHAT_ID
...
let criteriaText = "💥 *Критерии вступления*\nНеобходимо проявлять активность и готовность играть командно.";
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  switch(data) {
    ...
    case 'apply':
      await bot.answerCallbackQuery(callbackQuery.id);
      await bot.editMessageText("Отправьте ваше короткое представление и причину вступления.", {
        chat_id: chatId,
        message_id: messageId
      });
      break;
